#!/usr/bin/env python3
"""atlas_cli.py - Control ChatGPT Atlas from the terminal.

Commands:
    app-name    Print the detected Atlas application name
    tabs        List open Atlas tabs (--json for JSON output)
    open-tab    Open a new Atlas tab with a given URL
    focus-tab   Focus a tab by window_id and tab_index
    close-tab   Close a tab by window_id and tab_index
    reload-tab  Reload a tab by window_id and tab_index
    history     Search Atlas browsing history (--search, --today, --limit, --json)
    bookmarks   List or search Atlas bookmarks (--search, --limit, --json)

Requires macOS with ChatGPT Atlas installed.
"""
...
def _parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    bookmarks_parser.add_argument(
        "--limit",
        type=int,
        default=DEFAULT_BOOKMARK_LIMIT,
        help="Max rows to return",
    )
    bookmarks_parser.add_argument(
