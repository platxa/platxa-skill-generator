...
def parse_args():
    ...
    parser.add_argument(
        "--config", type=str, default="default", help="Dataset config name (default: default)"
    )
...
def api_request(url: str) -> dict:
    ...
    try:
        ...
        if exc.code == 404:
            return None
        raise Exception(f"API request failed: {exc.code} {exc.reason}") from exc
    except Exception as exc:
        raise Exception(f"API request failed: {str(exc)}") from exc
...
def main():
    ...
    try:
        ...
        if not splits_data or "splits" not in splits_data:
            ...
            print("       Dataset may not exist or is not accessible via Datasets Server API")
            sys.exit(1)
