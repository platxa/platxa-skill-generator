...
- Linux/macOS: Install from [render-oss/cli releases](https://github.com/render-oss/cli/releases)
...
2) Add this to `~/.cursor/mcp.json` (replace the API key placeholder):
```json
{
  "mcpServers": {
    "render": {
...
2) Add the MCP server with Claude Code (replace with their API key):
```bash
